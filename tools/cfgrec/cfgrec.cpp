...
static void DisassembleObject(const ObjectFile *Obj, bool InlineRelocs) {
  ...
  for (const SectionRef &Section : ToolSectionFilter(*Obj)) {
    ...
    for (unsigned si = 0, se = Symbols.size(); si != se; ++si) {
	  ...
	  // mcinst, address, target (0 if none)
	  std::vector<std::tuple<MCInst, unsigned, unsigned>> insts;
          ...
          if (MIA->evaluateBranch(Inst, SectionAddr + Index, Size, Target)) {
            ...
            if (TargetSectionSymbols) {
              ...
              if (TargetSym != TargetSectionSymbols->begin()) {
				...
				// nieuw stukje
				insts.push_back(std::make_tuple(Inst, Index, Disp-4));
				// nieuw stukje
                outs() << '>';
              }
            }
			...
			insts.push_back(std::make_tuple(Inst, Index, 0));
		}
        outs() << "\n";
	  ...
	  // nieuw stukje
	  std::vector<LabelledInst> linsts;
	  std::vector<unsigned> btargets; // all the branch targets
	  for (auto i : insts) {
		  LabelledInst linst;
		  linst.inst = std::get<0>(i);
		  linst.label = std::to_string(std::get<1>(i));
		  linst.origIndex = std::get<1>(i);
		  /* linst.label = ; */
		  if (std::get<2>(i)) {
			  linst.branchTarget = std::get<2>(i);
			  btargets.push_back(std::get<2>(i));
		  }
		  linsts.push_back(linst);
